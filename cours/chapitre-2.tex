...
Ces modèles permettent de capturer différentes structures de dépendance temporelle de manières plus ou moins  parcimonieuses.
\end{frame}
...
Cela n'affecte que l'espérance, pas les moments d'ordre 2 puisque nous ne faisons que rajouter une constante (déterministe):\newline
...
L'autocorrélation non nulle à l'ordre 1 apparaît car deux $Y$ consécutifs ont une innovation $\varepsilon$ en commun. Les couples de variables Y plus éloignées dans le temps ne partagent aucun $\varepsilon$.

\end{frame}

\begin{frame}{Densité spectrale du MA(1)}
\begin{block}{Rappel}
Pour un processus stationnaire, la densité spectrale est la transformée de Fourier de la fonction d'autocovariance :
\[
...
Puisque $\gamma(h) = 0$ pour $|h| \geq 2$, on a :
\begin{align*}
...
\textbf{Calcul :} Pour le MA(2), $Y_t = \varepsilon_t - \theta_1 \varepsilon_{t-1} - \theta_2 \varepsilon_{t-2}$, on a :
\[
f(\omega) = \frac{\sigma^2}{2\pi} \left| 1 - \theta_1 e^{-i\omega} - \theta_2 e^{-2i\omega} \right|^2
\]

En développant le module au carré :
\begin{align*}
...
\begin{alertblock}{Densité spectrale du MA(2)}
\[
...
Par définition de la fonction d'autocovariance obéit à la récurrence d'ordre 1 suivante~:
\[
...
avec $\gamma(0) = \dfrac{\sigma^2}{1 - \varphi^2}$. Cette équation récurrente est stable car $|\varphi|<1$.
...
\textbf{Calcul :} Pour le AR(1), $Y_t = \varphi Y_{t-1} + \varepsilon_t$, on a :
\[
...
\begin{alertblock}{Densité spectrale du AR(1)}
\[
...
\textbf{Remarque :} Comparer avec la densité spectrale du MA(1) :
\[
...
\begin{block}{Remarque}
Le processus $(X_t, t \in \Z)$ n'est pas l'innovation du processus $(Y_t, t \in \Z)$.
\end{block}
